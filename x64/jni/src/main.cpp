...
void libhook_main() {
	...
	my_cool_Patches.GodMode = MemoryPatch(libName, 0x7FF2E4, "\xfa\x0f\xa0\xe3\x1e\xff\x2f\xe1", 8);	//mov r0, #0x3e8 	bx  lr
	my_cool_Patches.GodMode.Modify();
	
	my_cool_Patches.BypassAnticheat = MemoryPatch(libName, 0x7FFCF8, "\x1e\xff\x2f\xe1", 4);	//bx lr
	my_cool_Patches.BypassAnticheat.Modify();
	
	//arm64 hook example.
	A64HookFunction((void *)getRealOffset(0x89552A), (void *)Player_getSkillCooldown, (void **) &old_Player_getSkillCooldown);
	A64HookFunction((void *)getRealOffset(0x89556C), (void *)Player_getWeaponDamage, (void **) &old_Player_getWeaponDamage);
}
